...
export async function getRosterByMatchId() {
  ...
  const [{ data: matchRows, error: e1 }, { data: teamEntries, error: e2 }, { data: regs, error: e3 }, { data: parts, error: e4 }, { data: deps, error: e5 }] =
    await Promise.all([
      ...
      sb.from("participants").select("id, full_name, departamental_id"),
  ...
  const partDeptById = {};
  const partFullNameById = {};
  (parts || []).forEach((p) => { partDeptById[p.id] = p.departamental_id; partFullNameById[p.id] = p.full_name; });
  ...
  // Categorías SIN team_entries (nadie anotado en equipo/pareja): pueden
  // ser categorías de UNA persona contra otra (Ajedrez, Tenis Singles,
  // etc. -- ver runSorteoForCategory en sorteoRunner.js). Se arma el
  // roster por persona, con la misma etiqueta "Nombre (Departamental)"
  // que usa personLabel() ahí, para que esos partidos también entren en
  // el chequeo de superposiciones reales.
  (regs || []).forEach((r) => {
    if (entriesByCategory[r.category_id]) return;
    const depName = depNameById[partDeptById[r.participant_id]];
    const fullName = partFullNameById[r.participant_id];
    if (!depName || !fullName) return;
    const label = `${fullName} (${depName})`;
    if (!rosterByLabel[r.category_id]) rosterByLabel[r.category_id] = {};
    rosterByLabel[r.category_id][label] = new Set([r.participant_id]);
  });
...
export async function resetSorteo({ disciplineId, categoryId } = {}) {
  ...
  // Todo en tandas chicas -- con "Todo el sorteo" categoryIds puede tener
  // 100+ ids, y mandarlos todos juntos en un solo .in(...) arriesga chocar
  // con el límite de largo de URL que usa PostgREST para armar el filtro.
  const categoryChunks = chunkArray(categoryIds, 40);

  let matchIds = [];
  for (const chunk of categoryChunks) {
    const { data: matchRows, error: e1 } = await sb.from("matches").select("id").in("category_id", chunk);
    if (e1) throw e1;
    matchIds = matchIds.concat((matchRows || []).map((m) => m.id));
  }

  for (const chunk of chunkArray(matchIds, 200)) {
    const { error: e2 } = await sb.from("match_results").delete().in("match_id", chunk);
    if (e2) throw e2;
  }
  for (const chunk of categoryChunks) {
    const { error: e3 } = await sb.from("matches").delete().in("category_id", chunk);
    if (e3) throw e3;
  }
  for (const chunk of categoryChunks) {
...
export async function applyCsvImportReplaceTotal(resolved, referenceData) {
  ...
  // inscripciones (una fila por cada fila válida del CSV). El participante_id
  // del CSV es solo un número correlativo del padrón, NO el id que usa la
  // base (participants.id es un uuid) -- se genera un id propio acá y se
  // usa después al armar las inscripciones, no el número del CSV.
  const idByParticipanteId = new Map();
  const participantsMap = new Map();
  resolved.valid.forEach((r) => {
    if (participantsMap.has(r.participanteId)) return;
    const dep = depByNorm.get(r.departamentalNorm);
    const id = randomUUID();
    idByParticipanteId.set(r.participanteId, id);
    participantsMap.set(r.participanteId, { id, full_name: r.fullName, departamental_id: dep ? dep.id : null });
  });
  ...
  const registrationsRows = resolved.valid.map((r) => ({
    participant_id: idByParticipanteId.get(r.participanteId),
