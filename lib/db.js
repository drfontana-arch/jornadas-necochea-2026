...
export async function getCategory(id) {
  const sb = getSupabase();
  const { data, error } = await sb.from("categories").select("*, disciplines(*)").eq("id", id).single();
  if (error) throw error;
  return data;
}
...
export const COPA_ORO_PLATA_DISCIPLINES = ["futbolReducido", "basquet", "voley", "hockey"];
...
export async function listTeamEntries(categoryId) {
  ...
  const { data, error } = await sb
    .from("team_entries")
    .select("*, departamentales(name)")
    .eq("category_id", categoryId)
    .order("team_number");
  if (error) throw error;
  return data.map((r) => ({ id: r.id, dept: r.departamentales.name, departamentalId: r.departamental_id, num: r.team_number }));
}
...
export async function listMatchesForCategory(categoryId, stage) {
  ...
  const { data, error } = await q.order("round");
  if (error) throw error;
...
export async function listAllMatches() {
  ...
  const { data, error } = await sb
    .from("matches")
    .select("*, categories(name, discipline_id, disciplines(name, duration_minutes))")
    .not("day", "is", null)
    .not("time", "is", null);
  if (error) throw error;
  return data.map((m) => ({
    ...fromDbMatch(m),
    disciplineId: m.categories.discipline_id,
    disciplineName: m.categories.disciplines.name,
    categoryName: m.categories.name,
    duration: m.categories.disciplines.duration_minutes,
  }));
}
...
export async function listAttendance() {
  const sb = getSupabase();
  const { data, error } = await sb.from("attendance").select("*, departamentales(name)");
  if (error) throw error;
  return data;
}
...
export async function listRestrictions() {
  ...
  if (error) throw error;
  return data.map((r) => ({ ...r, departamental_name: r.departamentales.name }));
}
export async function addRestriction(payload) {
  const sb = getSupabase();
  const { data, error } = await sb.from("schedule_restrictions").insert(payload).select("*, departamentales(name)").single();
  if (error) throw error;
  return { ...data, departamental_name: data.departamentales.name };
}
