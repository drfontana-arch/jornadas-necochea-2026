...
export function buildSlotPool(discipline, countNeeded, transitionMinutes, notBeforeMinutes = 0) {
  const slots = [];
  ...
  while (slots.length < countNeeded && safety < 300) {
    let algunaSedeConEspacio = false;
    for (let v = 0; v < discipline.venues.length && slots.length < countNeeded; v++) {
      const venue = discipline.venues[v];
      const startMinutes = parseHM(venue.time) + roundOffset * step;
      // Nunca dar la vuelta a medianoche: una sede/día no tiene horas infinitas.
      if (startMinutes >= 24 * 60) continue;
      if (startMinutes < notBeforeMinutes) { algunaSedeConEspacio = true; continue; }
      const endMinutes = startMinutes + discipline.duration;
      if (cutoff !== null && endMinutes > cutoff) continue;
      algunaSedeConEspacio = true;
      const t = fmtHM(startMinutes);
      for (let c = 1; c <= discipline.courts && slots.length < countNeeded; c++) {
        slots.push({ day: venue.day, time: t, court: c });
      }
    }
    if (!algunaSedeConEspacio) break; // ninguna sede tiene más lugar hoy; no tiene sentido seguir sumando rondas
    roundOffset++;
    safety++;
  }
  return slots;
...
export function assignSlotsAvoidingConflicts(matchesNeedingSlots, discipline, transitionMinutes, otherMatches, restrictions = [], notBeforeMinutes = 0) {
  const poolSize = matchesNeedingSlots.length * 4 + discipline.venues.length * discipline.courts * 2 + 6;
  const pool = buildSlotPool(discipline, poolSize, transitionMinutes, notBeforeMinutes);
...
export function scheduleRoundsProgressively(matchesNeedingSlots, discipline, transitionMinutes, baseOtherMatches, restrictions = [], initialNotBefore = 0) {
  ...
  roundNumbers.forEach((r) => {
    ...
    roundMatches.forEach((m) => {
      ...
      if (!s) return;
      const endMin = parseHM(s.time) + discipline.duration;
